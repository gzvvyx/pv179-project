...
{
    private record VideoSample(string Title, string Description, string Url);

    private static readonly VideoSample[] SampleVideos =
    [
        new(
            "Big Buck Bunny",
    ...
    {
        if (await db.Videos.AnyAsync()) return;
        ...
        var faker = new Faker<Video>()
            ...
            .RuleFor(v => v.Title, f => f.PickRandom(SampleVideos).Title)
            .RuleFor(v => v.Description, f => f.PickRandom(SampleVideos).Description)
            .RuleFor(v => v.Url, f => f.PickRandom(SampleVideos).Url)
            .RuleFor(v => v.ThumbnailUrl, f => $"https://picsum.photos/seed/{f.Random.Guid()}/1280/720")
